/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { AssignedAssetsWhereInput } from "./AssignedAssetsWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { AssignedAssetsOrderByInput } from "./AssignedAssetsOrderByInput";

@ArgsType()
class AssignedAssetsFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => AssignedAssetsWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => AssignedAssetsWhereInput, { nullable: true })
  @Type(() => AssignedAssetsWhereInput)
  where?: AssignedAssetsWhereInput;

  @ApiProperty({
    required: false,
    type: [AssignedAssetsOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [AssignedAssetsOrderByInput], { nullable: true })
  @Type(() => AssignedAssetsOrderByInput)
  orderBy?: Array<AssignedAssetsOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { AssignedAssetsFindManyArgs as AssignedAssetsFindManyArgs };
